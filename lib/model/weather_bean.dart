  ...
  factory WeatherBean.fromJson(Map<String, dynamic> json) {
    return WeatherBean(
        ...
        result: json['status'] == 0
            ? WeatherResult.fromJson(json['result'])
            : null);
  }
}
...
class WeatherResult {
  ...
  factory WeatherResult.fromJson(Map<String, dynamic> json) {
    ...
    List<WeatherIndex> indexList =
        temIndexs.map((i) => WeatherIndex.fromJson(i)).toList();

    var temDailys = json['daily'] as List;
    List<WeatherDaily> dailyList =
        temDailys.map((i) => WeatherDaily.fromJson(i)).toList();

    var temHours = json['hourly'] as List;
    List<WeatherHourly> hoursList =
        temHours.map((i) => WeatherHourly.fromJson(i)).toList();
